// 헤더 불러오기
fetch('/path/to/header/header.html')
  .then((res) => res.text())
  .then((data) => {
    ...
    document
      ...
      .setAttribute('src', `sub-hall/img/hdr-subhall-logo.svg`);
    document
      .querySelector('.header-include-hall .dropdown-icon')
      .setAttribute('src', `sub-hall/img/hdr-subhall-dropdown.svg`);
  ...
  on: {
    ...
    activeIndexChange: function () {
      ...
      hallLocationTapContent.forEach((a, i) => {
        a.setAttribute(
          'src',
          `sub-hall/img/${hallLocationTapMenu[this.realIndex].dataset.name}0${
            i + 1
          }.gif`
        );
      });
